    ...
    <main class="min-h-screen bg-[#f7f4ef] text-[#171717]">
        ...
        <template v-else-if="isTemplatePage && publicTemplate">
            ...
            <section id="builder" class="bg-white px-4 py-12 sm:px-6 lg:py-16">
                <div
                    class="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1.15fr_0.85fr]"
                >
                    <SiteConfigurator
        ...
        <template v-else-if="isPortfolioPage">
            <PortfolioPage
                :portfolio="siteContent.portfolio"
                :project-slug="portfolioProjectSlug"
            />
...
const isTemplatesListingPage = computed(() => {
    return currentPath === "/modele-site";
});

const isConfiguratorPage = computed(() => {
    return currentPath === "/configurator";
});

const isPortfolioPage = computed(() => {
    return (
        currentPath === "/portofoliu" || currentPath.startsWith("/portofoliu/")
    );
});

const portfolioProjectSlug = computed(() => {
    if (!isPortfolioPage.value || currentPath === "/portofoliu") {
        return "";
    }
...
const publicTemplate = computed(() => {
    return (
        templates.find((template) => {
            return template.slug === templateSlug.value;
        }) || null
    );
});
...
onMounted(() => {
    ...
    const storedTemplateId = Number(
        sessionStorage.getItem("sitego_selected_template_id"),
    );
    ...
    const storedTemplate = templates.find((template) => {
        return template.id === storedTemplateId;
    });
