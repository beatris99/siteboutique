    ...
    <footer class="border-t border-black/10 bg-black text-white">
        <div class="mx-auto grid max-w-7xl gap-10 px-6 py-14 md:grid-cols-2 lg:grid-cols-[1.25fr_0.8fr_0.8fr_0.8fr_0.8fr]">
            ...
            <div>
                ...
                <nav class="mt-5 grid gap-3 text-sm text-white/60">
                    <a
                        v-for="item in footer.service_links"
                        :key="item.href"
                        :href="item.href"
                        class="transition hover:text-white"
                    >
            ...
            <div>
                ...
                <nav class="mt-5 grid gap-3 text-sm text-white/60">
                    <a
                        v-for="item in footer.legal_links"
                        :key="item.href"
                        :href="item.href"
                        class="transition hover:text-white"
                    >
            ...
            <div>
                ...
                <div class="mt-5 grid gap-3 text-sm text-white/60">
                    ...
                    <p v-if="contactInfo.location">
                        {{ contactInfo.location }}
                    </p>

                    <div v-if="footer.social_links?.length" class="flex flex-wrap gap-4 pt-2">
                        <a
                            v-for="item in footer.social_links"
                            :key="item.href"
                            :href="item.href"
                            target="_blank"
                            rel="noopener"
