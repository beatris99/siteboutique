<template>
    <section id="newsletter" class="bg-[#f7f4ef] px-4 pb-14 sm:px-6 lg:pb-20">
        <div class="mx-auto max-w-6xl">
            <div class="rounded-[1.5rem] border border-black/10 bg-white px-5 py-5 shadow-sm sm:px-6 lg:flex lg:items-center lg:justify-between lg:gap-8">
                <div class="max-w-2xl">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.28em] text-[#a67c3a]">
                        {{ copy.compact_eyebrow }}
                    </p>

                    <h2 class="mt-2 text-xl font-semibold tracking-[-0.03em] text-[#171717] sm:text-2xl">
                        {{ copy.compact_title }}
                    </h2>
                ...
                <div class="mt-5 w-full lg:mt-0 lg:max-w-md">
                    ...
                    >
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row">
                            ...
                            >
                                {{ isSubmitting ? copy.sending : copy.submit_subscribe }}
                            </button>
                        </div>

                        <label class="flex items-start gap-3 text-xs leading-5 text-black/50">
                            <input
                                v-model="consent"
                                type="checkbox"
...
const fallback = {
    ...
    compact_title: 'Primește idei utile și coduri de lansare.',
...
const copy = computed(() => ({
    ...fallback,
    ...props.t,
    compact_eyebrow: props.t.compact_eyebrow || props.t.eyebrow || fallback.compact_eyebrow,
    compact_title: props.t.compact_title || props.t.title || fallback.compact_title,
    compact_description: props.t.compact_description || props.t.description || fallback.compact_description,
    messages: { ...fallback.messages, ...(props.t.messages || {}) },
}))
