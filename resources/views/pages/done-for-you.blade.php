...
<section class="grid gap-6">
    @foreach(__('pages.done_for_you.steps') as $step)
        <article class="rounded-[2rem] bg-[#f7f4ef] p-6"><p class="text-sm font-semibold text-[#805d2c]">{{ $step['number'] }}</p><h2 class="mt-3 text-2xl font-semibold">{{ $step['title'] }}</h2><p class="mt-3 text-black/60">{{ $step['description'] }}</p></article>
    @endforeach
</section>
